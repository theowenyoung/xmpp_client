...
class InboxNegotiator extends Negotiator {
  static const TAG = 'InboxNegotiator';

  static final Map<Connection, InboxNegotiator> _instances = {};

  static InboxNegotiator getInstance(Connection connection) {
    var instance = _instances[connection];
    if (instance == null) {
      instance = InboxNegotiator(connection);
  ...
  InboxNegotiator(this._connection) {
    expectedName = 'erlang-solutions.com:xmpp:inbox';
  }
  ...
  List<Nonza> match(List<Nonza> requests) {
    final list = requests
        .where((element) =>
            element is Feature &&
            ((element).xmppVar == 'erlang-solutions.com:xmpp:inbox:0'))
        .toList();
  ...
  void sendRequest() {
    ...
    query.addAttribute(
        XmppAttribute('xmlns', 'erlang-solutions.com:xmpp:inbox:0'));
